...
class BookAdapter (var context:Context, var list:ArrayList<Book>): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as BookHolder).bind(list[position].title,list[position].author,list[position].price,list[position].photo,list[position].id)
    }
    ...
    class BookHolder(itemView: View):RecyclerView.ViewHolder(itemView)
    {
        fun bind(t:String,a:String,p:Int,ph:String,book_id:Int)
        {
            ...
            //itemView.book_price.contentDescription = itemView.book_price.context.getString(R.string.local_ip)
            ...
            itemView.book_photo.setOnClickListener {
                ...
                UserInfo.bookTitle4blurb=itemView.book_title.text.toString()
